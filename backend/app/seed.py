...
Runs on startup so a fresh database always has sample data. Only seeds when the
products table is empty, so it is safe to call on every boot and harmless when a
persistent database already has data.
"""
...
async def seed_if_empty() -> None:
    """Populate demo products, price history and alerts if the DB is empty."""
    async with async_session() as db:
        ...
        for data in PRODUCTS:
            ...
            # 15 days of price history, ending at the current price.
            for day in range(14, -1, -1):
                ts = now - datetime.timedelta(days=day)
                price = (
                    data["current_price"]
                    if day == 0
                    else _random_price_around(data["original_price"])
                )
                db.add(PriceHistory(product_id=product.id, price=price, checked_at=ts))

            # A recent price-drop alert (within the last 2 days) so the dashboard
            # stats show real activity.
